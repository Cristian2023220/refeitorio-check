...
import { calcularSaldoMensal, type Aluno } from '../services/acesso.service';
...
async function buscarAluno(alunoId: string): Promise<Aluno> {
  const resultado = await query<Aluno>('SELECT * FROM alunos WHERE id = $1', [alunoId]);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    const resultado = await query<{ confirmado_em: Date }>(
...
router.post(
  '/confirmacoes',
  asyncHandler(async (req, res) => {
    const prazo = await query<{ valor: string }>("SELECT valor FROM configuracoes WHERE chave = 'prazo_confirmacao'");
...
router.get(
  ...
  asyncHandler(async (_req, res) => {
    const resultado = await query<{ valor: string }>("SELECT valor FROM configuracoes WHERE chave = 'cardapio_dia'");
