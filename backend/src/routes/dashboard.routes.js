...
router.get('/resumo', asyncHandler(async (req, res) => {
  const [confirmadosHoje, liberadosHoje, negadosHoje, saldoMedio] = await Promise.all([
    query(`SELECT COUNT(*)::int AS total FROM confirmacoes_presenca WHERE data = CURRENT_DATE`),
    query(`SELECT COUNT(*)::int AS total FROM registros_acesso WHERE resultado = 'liberado' AND lido_em::date = CURRENT_DATE`),
    query(`SELECT COUNT(*)::int AS total FROM registros_acesso WHERE resultado = 'negado' AND lido_em::date = CURRENT_DATE`),
    query(`
...
router.get('/confirmacoes', asyncHandler(async (req, res) => {
