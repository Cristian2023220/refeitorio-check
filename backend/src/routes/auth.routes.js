...
router.post('/login', asyncHandler(async (req, res) => {
  ...
  const resultado = await query('SELECT * FROM alunos WHERE matricula = $1 AND ativo = TRUE', [matricula]);
...
router.post('/admin/login', asyncHandler(async (req, res) => {
  ...
  const resultado = await query('SELECT * FROM usuarios_admin WHERE email = $1', [email]);
